...
int create_message(struct rdma_cm_id *id, int msg_type, int value)
{
	...
	void *ptr = NULL;
	int i = _mp_acquire_buffer(ctx->sockfd, &ptr, 0);
	...
	if(msg_type == MSG_MR) {
		...
		if(ctx->app_type >= 0) {
			ctx->msg_send[i]->meta.mr.type = ctx->app_type;
			ctx->msg_send[i]->meta.mr.addr = ctx->pid;
		}
		ctx->msg_send[i]->meta.mr.length = value;
		if(value > MAX_MR)
			mp_die("error; local mr count shouldn't exceed MAX_MR");
	}
	else
		mp_die("create message failed; invalid type");
	return i;
}
